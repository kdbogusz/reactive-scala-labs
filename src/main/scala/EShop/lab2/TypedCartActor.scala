...
object TypedCartActor {
  ...
  case class StartCheckout(
    actorEventMapper: ActorRef[TypedCartActor.Event],
    checkoutEventMapper: ActorRef[TypedCheckout.Event]
  ) extends Command
    ...
    (context, msg) =>
      msg match {
        ...
        case StartCheckout(actorEventMapper, checkoutEventMapper) =>
          timer.cancel()
          val checkout =
            context.spawn(new TypedCheckout(Some(context.self), Some(checkoutEventMapper)).start, "checkout")
